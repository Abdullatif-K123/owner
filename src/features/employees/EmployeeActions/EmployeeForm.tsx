...
import { EmployeeDefualts, employeeSchema } from "./validation";
...
type Props = {
  defaultValues?: EmployeePayload;
  defaultBranch?: BranchSelect;
};
const EmployeeForm = ({ defaultValues, defaultBranch }: Props) => {
  ...
  const {
    ...
    defaultValues: defaultValues ? defaultValues : EmployeeDefualts,
  ...
  const onSubmit = (data: EmployeePayload) => {
    mutate(data, {
      onSuccess: () => {
        ...
        queryClient.invalidateQueries([controllers.OwnerStaff, "details"]);
      },
      onError: (err) => {
        errorSnackbar(err);
      },
